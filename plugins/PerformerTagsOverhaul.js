(function () {
  ...
  function getHostDetailGroup() {
    const anchor = getInjectionAnchor();
    if (!anchor) return null;
    if (anchor.classList?.contains("detail-group")) return anchor;
    return anchor.closest(".detail-group") || null;
  }

  function markHostLayout() {
    const hostGroup = getHostDetailGroup();
    if (hostGroup) {
      hostGroup.classList.add("performer-tags-overhaul__host-group");
    }

    const detailHeader = hostGroup?.closest(".detail-header");
    if (detailHeader) {
      detailHeader.classList.add("performer-tags-overhaul__host-header");
    }
  }

  function clearHostLayout() {
    document
      .querySelectorAll(".performer-tags-overhaul__host-group")
      .forEach((el) => el.classList.remove("performer-tags-overhaul__host-group"));

    document
      .querySelectorAll(".performer-tags-overhaul__host-header")
      .forEach((el) => el.classList.remove("performer-tags-overhaul__host-header"));
  }
